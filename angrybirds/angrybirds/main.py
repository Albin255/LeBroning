...
while running:
    ...
    if help_menu_active:
        ...
        if adjusting_speed:
            if keys[pygame.K_LEFT]: ball_speed_x -= 0.2
            if keys[pygame.K_RIGHT]: ball_speed_x += 0.2
            if keys[pygame.K_UP]: ball_speed_y -= 0.2
            if keys[pygame.K_DOWN]: ball_speed_y += 0.2
        ...
        if game_started and not adjusting_speed:
            if (abs(ball_speed_x) < 0.5 and ball_speed_y == 0) or ball_y > screen_height or ball_rect.colliderect(thwomp_rect):
        ...
        if not targets:
            if remaining_shots == 2:
                ...
                draw_text("Victoire ! Tous les blocs sont détruits", 500, 300, 48, YELLOW, end_font_path)
